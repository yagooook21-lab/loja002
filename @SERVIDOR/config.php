				   ...
				   $sql = mysqli_query($conn, "SELECT * from config");
					 ...
					 $thefile = $imgFile;
							...
							$sql = mysqli_query($conn, "SELECT * FROM config");
							 ...
							 $app_mobile_ativo = isset($rowx["app_mobile_ativo"]) ? $rowx["app_mobile_ativo"] : "0";
							 $texto = $rowx["texto"];
							...
							echo '<li class="list-group-item border-0 d-flex align-items-center px-0 mb-2 pt-0">
							  ...
							  <div class="d-flex align-items-start flex-column justify-content-center">
								...
								<h6 class="mb-0 text-sm">Simulação App Mobile: '.($app_mobile_ativo == "1" ? "Ativo" : "Inativo").'</h6>
		...
		<div class="input-group input-group-outline my-3">
		 ...
		 <!--<p>Logo da loja</p>-->
		  ...
		  <input style="display:none; width:100%;text-align:center;color:transparent;" accept="image/*" type="file" id="favicon" name="favicon">
			...
			<div class="input-group input-group-outline my-3 is-filled" style="flex-direction: column;">
			  <label class="form-label" style="position: static; margin-bottom: 5px; width: 100%;">Simulação de App (Apenas Mobile)?</label>
			  <select id="app_mobile_ativo" class="form-control" style="padding: 10px; border: 1px solid #ccc; appearance: auto;">
			      <option value="1" <?php if($app_mobile_ativo == "1") echo "selected"; ?>>Sim</option>
			      <option value="0" <?php if($app_mobile_ativo == "0") echo "selected"; ?>>Não</option>
			  </select>
			</div>
			<div class="input-group input-group-outline my-3 is-filled">
...
<script>
	 ...
	 var app_mobile_ativo = document.getElementById("app_mobile_ativo").value;
	 ...
	 if(!cordaloja || !nomedaloja || !numerozap || !textozap){
	  ...
	  document.getElementById("infoToast1").setAttribute("class","toast fade hide p-2 mt-2 bg-gradient-danger hide");
		 ...
		 $.post("api_adm/", {painel:"atualizarLoja", cor:cordaloja, cor_botao:corbotao, cor_icones:coricones, nome:nomedaloja, zap:numerozap, zap_cotacao:zap_cotacao, zap_flutuante_ativo:zap_flutuante_ativo, app_mobile_ativo:app_mobile_ativo, texto:textozap, endereco:endereco, cnpj:cnpj},function(zeragem){
