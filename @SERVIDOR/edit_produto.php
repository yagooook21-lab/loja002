  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
    <div class="container-fluid py-4">
      <div class="card card-body mx-3 mx-md-4 mt-2">
        ...
        <div class="row mt-4">
          <!-- Seção do Atualizador -->
          <div class="col-12 mb-4">
            <div class="url-input-group" style="background: #f0f2f5; border: 2px dashed #3483fa;">
              <h6><i class="material-icons" style="vertical-align: middle;">sync</i> Atualizar dados do produto</h6>
              <p class="text-xs text-secondary">Abra a página do Mercado Livre, pressione <code>CTRL + U</code>, copie o código fonte e cole abaixo para sobrepor as informações atuais (MANTENDO O MESMO LINK DA SUA CAMPANHA).</p>
              <textarea id="produto_source" class="form-control mb-3" rows="4" placeholder="Cole o código fonte (HTML) aqui..."></textarea>
              <button type="button" class="btn bg-gradient-info btn-sm" onclick="importarProduto();">Atualizar Dados</button>
            </div>
          </div>
                ...
                <div class="col-md-6">
                  <div class="input-group input-group-outline my-3 is-filled">
                    ...
                    <select id="tipo_produto" name="tipo_produto" class="form-control" onchange="atualizarVariacoes();">
                      ...
                      <option value="outros" <?php echo (($prod['tipo_produto'] ?? '') == 'outros' ? 'selected' : ''); ?>>Outros (apenas cores)</option>
                    </select>
                  </div>
                </div>
              </div>
...
v=v.replace(/\D/g,'');
    ...
    function importarProduto() {
      ...
      let srcs = [...new Set(galleryImgs.map(i => i.src || i.dataset.src || i.getAttribute('data-zoom')).filter(s => s && !s.includes('pixel') && s.startsWith('http') && !s.endsWith('.svg')))];
      
      // Buscar video no codigo fonte (YouTube)
      const ytMatch = raw.match(/https:\/\/(?:www\.)?youtube\.com\/embed\/([a-zA-Z0-9_-]+)/i) || 
                      raw.match(/"videoId":"([a-zA-Z0-9_-]+)"/);
      
      if (ytMatch && ytMatch[1]) {
          const videoUrl = "https://www.youtube.com/embed/" + ytMatch[1];
          // Adiciona o video na segunda posicao
          if (srcs.length > 0) {
              srcs.splice(1, 0, videoUrl);
          } else {
              srcs.push(videoUrl);
          }
      } else {
          // Buscar video nativo do Mercado Livre (Shorts / m3u8)
          const m3u8Match = raw.match(/"video_url"\s*:\s*"([^"]+\.m3u8)"[\s\S]*?"thumbnail"\s*:\s*\{[\s\S]*?"url"\s*:\s*\{[\s\S]*?"src"\s*:\s*"([^"]+)"/);
          if (m3u8Match) {
              const rawVideoUrl = m3u8Match[1].replace(/\\u002F/g, '/');
              const rawThumbUrl = m3u8Match[2].replace(/\\u002F/g, '/');
              const videoUrl = "m3u8_video|" + rawVideoUrl + "|" + rawThumbUrl;
              
              let replaced = false;
              for (let j=0; j<srcs.length; j++) {
                  let tName = rawThumbUrl.split('/').pop().split('.')[0];
                  let sName = srcs[j].split('/').pop().split('.')[0];
                  if (tName === sName) {
                      srcs[j] = videoUrl;
                      replaced = true;
                      break;
      ...
      // Tenta acionar a mascara de preço (se houver)
      if (typeof jQuery !== 'undefined' && $.fn.mask) {
        $('#valor').trigger('input');
      }
      
      alert("Dados extraídos com sucesso! Revise os campos e clique em Salvar para atualizar as informações deste produto.");
    }
  </script>
</body>
</html>
